...
db.serialize(() => {
    ...
    db.run(`
        CREATE TABLE IF NOT EXISTS quotation_items (
            ...
            unit TEXT DEFAULT '',
            qty REAL DEFAULT 0,
            install_price REAL DEFAULT 0,
            equip_price REAL DEFAULT 0,
            remark TEXT DEFAULT '',
...
app.get('/api/quotations', (req, res) => {
    db.all(
        `SELECT id, name, date, quoter, tax_rate, created_at, updated_at 
         FROM quotations 
...
app.post('/api/quotations', (req, res) => {
    const { name, date, quoter, tax_rate, items } = req.body;
    const now = new Date().toISOString();

    db.run(
        `INSERT INTO quotations (name, date, quoter, tax_rate, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [name || '', date || '', quoter || '', tax_rate || 6, now, now],
        function(err) {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
            ...
            if (items && items.length > 0) {
                const stmt = db.prepare(
                    `INSERT INTO quotation_items 
                     (quotation_id, sort_order, name, model, unit, qty, install_price, equip_price, remark)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
                );
...
app.put('/api/quotations/:id', (req, res) => {
    ...
    db.run(
        `UPDATE quotations 
         SET name = ?, date = ?, quoter = ?, tax_rate = ?, updated_at = ?
         WHERE id = ?`,
        [name || '', date || '', quoter || '', tax_rate || 6, now, id],
        function(err) {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
